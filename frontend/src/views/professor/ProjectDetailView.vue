        ...
        <table v-else class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="sub in submissions" :key="sub.id" class="hover:bg-slate-50/50 transition-colors group">
              ...
              <td class="px-6 py-4 text-center">
                <div v-if="sub.grade !== null && sub.grade !== undefined" class="text-sm font-black text-slate-900">
                  {{ sub.grade }}
                </div>
...
const loadData = async () => {
  ...
  try {
    ...
    project.value = projData?.data || projData;
    submissions.value = Array.isArray(subData.submissions) ? subData.submissions : [];
    stats.value = statData?.data || statData;
    roster.value = Array.isArray(rosterData) ? rosterData : (rosterData?.data || []);
    milestones.value = msData?.data?.data || msData?.data || [];
  } catch (err) {
